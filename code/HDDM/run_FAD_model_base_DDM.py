...
dataPath = str(paths.fad_respons_data) 

# specify where to save models
model_loc = paths.fad_models
...
data = data[['subject', 'state', 'rt', 'response', 'emotion']] 

#Rename columns to fit expected names
data = data.rename({'subject': 'subj_idx'}, axis=1)
...
for p in params:
        if p == 'v':
                specifications.append('v ~ 0 + C(emotion, Treatment(0)) + C(emotion, Treatment(0)):C(state, Treatment(0))')
        else:
...
model_name = f'15_FAD_base_{nmcmc}_{burn}_{thin}_{chains}_{chain}'
print(model_name)
